...
TrashCat 的核心不是“扫描出尽可能多的 GB”，而是“准确判断哪些空间可以安全释放，哪些只是需要解释的大空间占用”。

产品必须减少用户选择压力：
- 用户不应该靠阅读 `~/Library/...` 路径来判断能不能删。
- 默认清理只包含确定安全、可重建、低风险的内容。
- iOS 备份、Xcode Archives、Photos、Mail、聊天软件、虚拟机等大空间占用应优先作为空间诊断，不应混进一键清理。
- 应用残留在规则成熟前统一称为“可能的应用残留”，默认不选中。
...
| US-02 | 作为 Mac 用户，我希望能**一键清理确定安全的垃圾文件**，以便快速释放磁盘空间 | P0 |
| US-03 | 作为 Mac 用户，我希望能看到**可能的应用残留和大空间占用说明**，以便知道空间被谁占了 | P0 |
| US-04 | 作为 Mac 用户，我希望能按**风险层级**（推荐清理/需要确认/谨慎处理）查看结果，以便不用自己判断复杂路径 | P0 |
| US-05 | 作为 Mac 用户，我希望清理前能看到**删除影响说明**，以便确认不会误删重要数据 | P1 |
...
| 废纸篓 | `~/.Trash/` | 有时候清不干净 |
| 可能应用残留 | `~/Library/Preferences/`、`~/Library/Application Support/`、`~/Library/Containers/`、`~/Library/Group Containers/` | 核心诊断功能，默认不清理 |

#### 3.1.1 扫描分层

| 层级 | 默认动作 | 示例 |
|------|----------|------|
| 推荐清理 | 默认勾选 | 明确缓存、旧日志、过期临时文件、浏览器 cache 子目录 |
| 需要确认 | 默认不选 | Xcode DerivedData、包管理器缓存、可重建但代价较高的数据 |
| 空间诊断 / 谨慎处理 | 默认不选，初期只展示 | iOS 备份、Xcode Archives、Docker volumes、聊天软件文件、虚拟机镜像 |
...
4. 将无法匹配到任何已安装应用的 bundle ID 的文件/目录标记为「可能残留」
5. 安全白名单：排除 Apple 系统应用相关、排除已知安全路径
6. 默认不选中可能残留，直到规则能解释来源和删除影响
```
...
| 风险分层展示 | 按「推荐清理」「需要确认」「谨慎处理」作为主视图 | P0 |
...
| 默认清理用户数据 | 备份、照片、邮件、聊天记录、虚拟机等只做诊断，不混入一键清理 |
| 扫描系统关键路径 | `/System`、`/usr`、`/bin`、`/sbin`、`/private/var/db`、Keychains 等不进入普通清理结果 |
...
- 删除操作使用 `FileManager.trashItem`（移到废纸篓，可恢复）而非 `removeItem`（永久删除）
  ...
  - 后续可以加「深度清理」选项使用永久删除
- 引入规则层 `CleanRule` / `ScanPolicy`，每条规则必须声明风险等级、默认动作、删除代价、删除方式
- 对系统工具有官方清理入口的场景优先调用官方方式，例如模拟器、Docker、包管理器缓存
